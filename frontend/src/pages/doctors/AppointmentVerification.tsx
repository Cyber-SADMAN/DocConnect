import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import axios from '../../api/axios';
import RootLayout from '../../components/layouts/RootLayout';
import toast from 'react-hot-toast';
import { faCheckCircle } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';

const AppointmentVerification = () => {
    const _timer = 60;

    const [verificationCode, setVerificationCode] = useState('');
    const [timer, setTimer] = useState(_timer);

    const [disableInput, setDisableInput] = useState(false);
    const [loading, setLoading] = useState(false);
    const [resendLoading, setResendLoading] = useState(false);
    const [completed, setCompleted] = useState(false);

    const navigate = useNavigate();

    const appointmentId = localStorage.getItem('appointmentId') || '';

    useEffect(() => {
        if (!appointmentId || appointmentId === '') {
            navigate('/');
        }

        const countdown = setInterval(() => {
            setTimer((prev) => {
                if (prev > 0) {
                    return prev - 1;
                }

                setDisableInput(true);
                clearInterval(countdown);
                return 0;
            });
        }, 1000);

        return () => clearInterval(countdown);
    }, []);

    const formatTimer = (seconds: number): string => {
        const minutes = Math.floor(seconds / 60);
        const secs = seconds % 60;
        return `${minutes.toString().padStart(2, '0')}:${secs
            .toString()
            .padStart(2, '0')}`;
    };

    const handleVerification = async () => {
        try {
            if (verificationCode.trim() === '') {
                toast.error('Please enter verification code.');
                return;
            }
            setDisableInput(true);
            setLoading(true);
            setResendLoading(true);

            const response = await axios.post('/appointments/verify-code', {
                code: verificationCode,
                appointmentId,
                currentTime: new Date().toISOString(),
            });

            if (response.data.success) {
                toast.success('Appointment verified successfully.');
                localStorage.removeItem('appointmentId');
                setCompleted(true);
            } else {
            }
        } catch (error: any) {
            console.error('Error booking appointment:', error);
            if (typeof error.response.data.message === 'string') {
                toast.error(error.response.data.message);
            } else {
                const key = Object.keys(error.response.data.message)[0];
                toast.error(error.response.data.message[key]);
            }
        } finally {
            setDisableInput(false);
            setLoading(false);
            setResendLoading(false);
        }
    };

    const handleResendCode = async () => {
        try {
            setDisableInput(true);
            setResendLoading(true);

            await axios.post('/appointments/resend-code', {
                appointmentId,
            });
            setTimer(_timer);
        } catch (error: any) {
            console.log('Error obj_str: ', error.response.data.message);
            if (typeof error.response.data.message === 'string') {
                alert('string');
                toast.error(error.response.data.message);
            } else {
                alert('object');
                const key = Object.keys(error.response.data.message)[0];
                toast.error(error.response.data.message[key]);
            }
        } finally {
            setDisableInput(false);
            setResendLoading(false);
        }
    };

    return (
        <RootLayout>
            {completed ? (
                <div className="">
                    <h2 className="text-2xl font-semibold mb-4 text-green-700">
                        <FontAwesomeIcon
                            icon={faCheckCircle}
                            className="mr-3"
                        />
                        Appointment Verified
                    </h2>
                    <p className="text-gray-600 mb-4">
                        Thank you for verifying your appointment. A confirmation
                        email with all the details has been sent to your email
                        address. We look forward to seeing you soon.
                    </p>
                </div>
            ) : (
                <div className="bg-white shadow-md rounded-lg p-3 md:p-6 mt-4">
                    <h3 className="font-semibold text-2xl mb-4">
                        Verify Appointment
                    </h3>

                    <div className="mb-2 flex items-center justify-between">
                        <label htmlFor="code" className="block font-semibold">
                            Verification Code
                        </label>
                        <span className="text-gray-600">
                            {formatTimer(timer)}
                        </span>
                    </div>
                    <input
                        type="text"
                        id="code"
                        value={verificationCode}
                        onChange={(e) => setVerificationCode(e.target.value)}
                        className={`w-full px-3 py-2 rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                            disableInput ? 'bg-gray-200 cursor-not-allowed' : ''
                        }`}
                        placeholder="Enter verification code"
                        readOnly={disableInput}
                        required
                    />
                    <button
                        type="button"
                        onClick={handleVerification}
                        className={`w-full py-2 bg-blue-500 text-white font-semibold rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 mt-4 ${
                            loading || disableInput ? ' cursor-not-allowed' : ''
                        }`}
                        disabled={loading || disableInput}
                    >
                        {loading ? 'Verifying...' : 'Verify Code'}
                    </button>
                    {timer === 0 ? (
                        <p className="mt-4 text-gray-600">
                            Didn't receive a code?{' '}
                            <button
                                type="button"
                                onClick={handleResendCode}
                                className="text-blue-500 underline focus:outline-none"
                                disabled={resendLoading}
                            >
                                {resendLoading ? (
                                    <div className="flex items-center gap-1">
                                        Resending
                                        <div role="status">
                                            <svg
                                                aria-hidden="true"
                                                className="w-5 h-5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
                                                viewBox="0 0 100 101"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                            >
                                                <path
                                                    d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                                                    fill="currentColor"
                                                />
                                                <path
                                                    d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                                                    fill="currentFill"
                                                />
                                            </svg>
                                            <span className="sr-only">
                                                Loading...
                                            </span>
                                        </div>
                                    </div>
                                ) : (
                                    'Resend'
                                )}
                            </button>
                        </p>
                    ) : (
                        <p className="mt-4 text-yellow-700">
                            *NB: Enter the code sent to your email
                        </p>
                    )}
                </div>
            )}
        </RootLayout>
    );
};

export default AppointmentVerification;
